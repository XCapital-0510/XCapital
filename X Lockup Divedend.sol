...
contract CommonFunc is Ownable {
    ...
    function setBlackList(address account, uint256 braveId)
        external
    {
        require(msg.sender == owner());
        blackList[account] = braveId;
    }
...
contract XReward is CommonFunc {
    using SafeMath for uint256;
    ...
    //奖金池领取功能
    mapping (address => uint256) public nextRewardTime; //下次领奖时间 值为0则不会触发领奖

    uint256 public discount = 80;//奖金池80%作为奖金

    uint256 public rewardLimitPeriod = 2; //领奖周期为48小时
