...
contract CommonFunc is Ownable {
    ...
    function setBlackList(address account, uint256 XId)
        external
    {
        require(msg.sender == owner());
        blackList[account] = XId;
    }
...
contract XMine is CommonFunc {
    ...
    event XMineBegin(uint256 tradeCode, address indexed from);

    event XMineEnd(uint256 tradeCode, address indexed from);
        ...
        public
    {
        ...
        require(
            ...
            nftPledgeTokenNeed[XOriginNFT(nftAddr).XIds(_tokenId)]
        );
        ...
        tradeList[tradeCodeCounter].XId = XOriginNFT(nftAddr).XIds(_tokenId);
        ...
        emit XMineBegin(tradeCodeCounter, msg.sender);
    ...
    function setNftPledgeTokenNeed(uint256 _XId, uint256 _amount)
        public
    {
        require(msg.sender == owner());

        nftPledgeTokenNeed[_XId] = _amount;
    }
        ...
        returns (uint256 mintReward)
    {
        if(block.timestamp < tradeList[_tradeCode].createTime+maxDays.mul(86400)) {
            mintReward = mintDays.mul(XOriginNFT(tradeList[_tradeCode].nftAddr)
                    .getMintSpeed(tradeList[_tradeCode].tokenId))
                    .mul(tradeList[_tradeCode].tokenAmount)
                    .div(tradeList[_tradeCode].rewardBase);
                    
        }else {
            mintReward = maxDays.mul(XOriginNFT(tradeList[_tradeCode].nftAddr)
                    .getMintSpeed(tradeList[_tradeCode].tokenId))
                    .mul(tradeList[_tradeCode].tokenAmount)
                    .div(tradeList[_tradeCode].rewardBase);
        }
        ...
        public
    {
        ...
        emit XMineEnd(_tradeCode, tradeList[_tradeCode].owner);
    }
        ...
        public
    {
        ...
        require(
            IBEP20(pledgeTokenAddr).balanceOf(msg.sender) >= 
            nftPledgeTokenNeed[XOriginNFT(nftAddr).XIds(tradeList[_tradeCode].tokenId)]
        );
