...
contract XToken is Context, IBEP20, Ownable {
    ...
    //奖金池地址
    //彩票池地址+奖励池的BNB地址统一为合约地址
    address private addressAD = 0x3A9f4021365c6117bDfE0095604959931D1fA6e6;//空投地址
    ...
    //uint256 public leaderTokenNeed;
    ...
    function _transfer(
        ...
        //获取pair地址
        address pancakePair = BraveCommon.getPairAddress(pancakeRouter, address(this), usdt);
        ...
        if(from == pancakePair) {
            if(to == address(pancakeRouter)) {
                ...
                //买入Brave时，pancake的交易输出地址必须为AD地址
                //if(to == address(addressAD) && swapUnlock) {
                if(swapUnlock) {
    ...
    /*
    function setLeaderTokenNeed(uint256 amount) public {
        require(tx.origin == owner());
        leaderTokenNeed = amount;
    }
    */
    ...
    //卖出Brave的接口，含Brave换取BNB的逻辑，卖出Brave时，pancake接口的交易输入地址必须为合约地址
    ...
    /*
    function getPairCondition() public view returns (uint256 pairUSDTAmount, uint256 pairTokenAmount, uint256 tokenTotal) {
        //获取pair地址
        address pancakePair = BraveCommon.getPairAddress(pancakeRouter, address(this), usdt);
        if(pancakePair == address(0)) {
            ...
            pairUSDTAmount = IBEP20(usdt).balanceOf(pancakePair);
            pairTokenAmount = _tOwned[pancakePair];
        }
        tokenTotal = _tTotal;
    }

    //当前钱包地址持有BraveToken代币数量
    function getCurrency() public view returns (uint256) {
        address pancakePair = BraveCommon.getPairAddress(pancakeRouter, address(this), usdt);
        return _tTotal.sub(_tOwned[rewardPool]).sub(_tOwned[minePool]).sub(_tOwned[pancakePair]);
    }


    function getPancakeUSDTTokenPair(address tokenAddress) public view returns (address) {
        return BraveCommon.getPairAddress(pancakeRouter, tokenAddress, usdt);
    }
    */
